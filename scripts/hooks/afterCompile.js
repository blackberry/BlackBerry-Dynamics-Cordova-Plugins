...
module.exports = function(context) {
    ...
    // Removes Window "clobbers", "merges" from cordova-plugin-file to avoid conflicts with cordova-plugin-bbd-file
    // when both plugins are added
    // DEVNOTE: cordova-plugin-file API's can be accessed using cordova.require('cordova-plugin-file.someMethod')
    function handlePlatformCordovaPlugins(cordovaPluginsPath) {
        ...
        // Update propertires for cordova-plugin-file entries
        var updatedPluginList = pluginsList.map(function(plugin) {
            if (plugin.pluginId === 'cordova-plugin-file') {
