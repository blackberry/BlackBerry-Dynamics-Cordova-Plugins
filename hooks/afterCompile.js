...
module.exports = function(context) {
    ...
    // Removes Window "clobbers", "merges" from cordova-plugin-media-capture to avoid conflicts with cordova-plugin-bbd-media-capture
    // when both plugins are added
    // DEVNOTE: cordova-plugin-media-capture API's can be accessed using cordova.require('cordova-plugin-media-capture.someMethod')
    function handlePlatformCordovaPlugins(cordovaPluginsPath) {
        ...
        // Update propertires for cordova-plugin-media-capture entries
        var updatedPluginList = pluginsList.map(function(plugin) {
            if (plugin.pluginId === 'cordova-plugin-media-capture') {
