...
-(void)openNow: (CDVInvokedUrlCommand*)command
{
    ...
    if (!sqlite3_threadsafe()) {
        ...
        @synchronized(self) {
            ...
            if (sqlite3enc_open(name, &db) != SQLITE_OK) {
...
-(CDVPluginResult*) executeSqlWithDict: (NSMutableDictionary*)options andArgs: (NSMutableDictionary*)dbargs
{
    ...
    if (sqlite3_prepare_v2(db, sql_stmt, -1, &statement, NULL) != SQLITE_OK) {
        error = [SQLitePluginBD captureSQLiteErrorFromDb:db];
        keepGoing = NO;
    } else if (params != NULL) {
...
+(NSDictionary *)captureSQLiteErrorFromDb:(struct sqlite3 *)db
{
    ...
    int webSQLCode = [SQLitePluginBD mapSQLiteErrorCode:code];
...
#pragma mark - GD SQLite3

//****************************** GD sqlite3enc_import *****************************
-(void)sqlite3enc_import:(CDVInvokedUrlCommand*)command
{
    NSString* callbackId = command.callbackId;
    NSString* srcFilename = [SQLitePluginBD fullPathWithAppkineticsPath:[command.arguments objectAtIndex: 0]];
    NSString* destFilename = [SQLitePluginBD fullPathWithStoragePath:[command.arguments objectAtIndex: 1]];
    CDVPluginResult* result = nil;
    int resultCode = sqlite3enc_import([srcFilename UTF8String], [destFilename UTF8String]);

    if (resultCode == SQLITE_OK) {
        NSMutableDictionary* dbFileEntry = [NSMutableDictionary dictionaryWithCapacity:5];
        NSString* name = [destFilename lastPathComponent];
        NSURL* nativeURL = [NSURL fileURLWithPath:[self filesystemPathForFullPath:destFilename]];

        [dbFileEntry setObject:[NSNumber numberWithBool:YES] forKey:@"isFile"];
        [dbFileEntry setObject:[NSNumber numberWithBool:NO] forKey:@"isDirectory"];
        [dbFileEntry setObject:[SQLitePluginBD relativePathFromFullPath:destFilename] forKey:@"fullPath"];
        [dbFileEntry setObject:name forKey:@"name"];
        [dbFileEntry setObject:[nativeURL absoluteString] forKey:@"nativeURL"];

        result = [CDVPluginResult resultWithStatus: CDVCommandStatus_OK messageAsDictionary:dbFileEntry];
    } else {
...
- (NSString *)filesystemPathForFullPath:(NSString *)fullPath
{
    ...
    path = [NSString stringWithFormat:@"%@%@", @"/", strippedFullPath];
