...
#pragma mark Server trust section
...
- (void)displayServerTrustDialogFromError:(NSError *)error
{
    serverTrustError = error;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"Deny"
                                                                  ...
                                                                  handler:^(UIAlertAction * _Nonnull action) {
                                                                        [weakSelf showUntrustedResourceError:error];
                                                                        [weakAlertController
...
- (void)showUntrustedResourceError:(NSError *)error
{
    [self.spinner stopAnimating];
    self.addressLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    self.addressLabel.text = NSLocalizedString([error localizedDescription], nil);
}
...
{
    NSURLRequest* request = [NSURLRequest requestWithURL:[[self currentURL] absoluteURL]];
    [self.webView loadRequest:request];
}
