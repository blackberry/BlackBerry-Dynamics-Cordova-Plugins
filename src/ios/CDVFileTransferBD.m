...
@interface CDVFileTransferBD ()
// Sets the requests headers for the request.
- (void)applyRequestHeaders:(NSDictionary*)headers toRequest:(NSMutableURLRequest*)req;
// Creates a delegate to handle an upload.
- (CDVFileTransferDelegateBD*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command;
...
@implementation CDVFileTransferBD
...
- (void)applyRequestHeaders:(NSDictionary*)headers toRequest:(NSMutableURLRequest*)req
{
    ...
    // DEVNOTE: related to issue "File upload breaking with Error code - 3 When updated from v1.7.2-dev to 2.0.0-dev"
    // https://github.com/apache/cordova-plugin-file-transfer/issues/285
...
- (NSURLRequest*)requestForUploadCommand:(CDVInvokedUrlCommand*)command fileData:(NSData*)fileData
{
    ...
    CDVFileTransferErrorBD errorCode = 0;
...
- (CDVFileTransferDelegateBD*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFileTransferDelegateBD* delegate = [[CDVFileTransferDelegateBD alloc] init];
...
- (void)fileDataForUploadCommand:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFilesystemURLBD *sourceURL = [CDVFilesystemURLBD fileSystemURLWithString:source];
    NSObject<CDVFileSystemBD> *fs;
    if (sourceURL) {
    ...
    if (fs) {
        __weak CDVFileTransferBD* weakSelf = self;
        [fs readFileAtURL:sourceURL start:0 end:-1 callback:^(NSData *fileData, NSString *mimeType, CDVFileErrorBD err) {
            if (err) {
...
- (void)uploadData:(NSData*)fileData command:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFileTransferDelegateBD* delegate = [self delegateForUploadCommand:command];
    delegate.session = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]
                                                   delegate:delegate
                                              delegateQueue:[NSOperationQueue mainQueue]];

    NSURLSessionDataTask *dataTask = [delegate.session dataTaskWithRequest:req];
    ...
    delegate.backgroundTaskID = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
        [delegate cancelSessionTask:dataTask];
    }];
...
- (void)abort:(CDVInvokedUrlCommand*)command
{
    ...
    @synchronized (activeTransfers) {
        CDVFileTransferDelegateBD* delegate = activeTransfers[objectId];
        if (delegate != nil) {
...
- (void)download:(CDVInvokedUrlCommand*)command
{
    ...
    if (!sourceURL) {
        ...
        CDVFilesystemURLBD *fsURL = [CDVFilesystemURLBD fileSystemURLWithString:target];
        if (!fsURL) {
            errorCode = FILE_NOT_FOUND_ERR;
...
- (void)onReset {
    @synchronized (activeTransfers) {
        while ([activeTransfers count] > 0) {
            CDVFileTransferDelegateBD* delegate = [activeTransfers allValues][0];
            [delegate cancelSessionTask:delegate.sessionTaskConnection];
        }
    }
}
...
@interface CDVFileTransferEntityLengthRequestBD : NSObject
{
    CDVFileTransferDelegateBD* __weak _originalDelegate;
    NSURLSessionDataTask* _dataTaskSession;
}

- (CDVFileTransferEntityLengthRequestBD*)initWithOriginalRequest:(NSURLRequest*)originalRequest andDelegate:(CDVFileTransferDelegateBD*)originalDelegate;

@end

@implementation CDVFileTransferEntityLengthRequestBD
...
- (CDVFileTransferEntityLengthRequestBD*)initWithOriginalRequest:(NSURLRequest*)originalRequest andDelegate:(CDVFileTransferDelegateBD*)originalDelegate
{
    if (self) {
        ...
        _dataTaskSession = [session dataTaskWithRequest:req];
    }
    return self;
}
...
@implementation CDVFileTransferDelegateBD

@synthesize callbackId, source, target, responseData, responseHeaders, command, bytesTransfered, bytesExpected, direction, responseCode, objectId, targetFileHandle, filePlugin, sessionTaskConnection = _sessionTaskConnection;

- (void)removeTargetFile
{
    GDFileManager* fileMgr = [GDFileManager defaultManager];
...
- (void)cancelSessionTask:(NSURLSessionDataTask*) dataTask
{
    ...
    @synchronized (self.command.activeTransfers) {
        ...
        CDVFileTransferDelegateBD* delegate = self.command.activeTransfers[self.objectId];
...
{
    NSString *path = nil;
    CDVFilesystemURLBD *sourceURL = [CDVFilesystemURLBD fileSystemURLWithString:self.target];
    if (sourceURL && sourceURL.fileSystemName != nil) {
        // This requires talking to the current CDVFile plugin
        NSObject<CDVFileSystemBD> *fs = [self.filePlugin filesystemForURL:sourceURL];
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    NSMutableDictionary* downloadProgress = [NSMutableDictionary dictionaryWithCapacity:3];
    [downloadProgress setObject:[NSNumber numberWithBool:lengthComputable] forKey:@"lengthComputable"];
    [downloadProgress setObject:[NSNumber numberWithLongLong:bytesWritten] forKey:@"loaded"];
    [downloadProgress setObject:[NSNumber numberWithLongLong:totalBytesWritten] forKey:@"total"];
...
didCompleteWithError:(nullable NSError *)error
{
    ...
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        if (self.targetFileHandle) {
            ...
            CDVFileTransferErrorBD errorCode = self.responseCode == 404 ? FILE_NOT_FOUND_ERR
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend
{
    ...
    [uploadProgress setObject:[NSNumber numberWithUnsignedLongLong:totalBytesSent] forKey:@"loaded"];
    [uploadProgress setObject:[NSNumber numberWithUnsignedLongLong:totalBytesExpectedToSend] forKey:@"total"];
 ...
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler
{
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if ((self.direction == CDV_TRANSFER_DOWNLOAD) && (self.responseCode == 200) && (self.bytesExpected == NSURLResponseUnknownLength)) {
            ...
            NSURL* sourceURL = [NSURL URLWithString:source];
            NSURLRequest* requet = [NSURLRequest requestWithURL:sourceURL];
            self.entityLengthRequest =[[CDVFileTransferEntityLengthRequestBD alloc] initWithOriginalRequest:requet andDelegate:self];
        }
    } else if ([response.URL isFileURL]) {
        NSDictionary* attr = [[GDFileManager defaultManager] attributesOfItemAtPath:[response.URL path] error:nil];
    ...
    if ((self.direction == CDV_TRANSFER_DOWNLOAD) && (self.responseCode >= 200) && (self.responseCode < 300)) {
        ...
        if (filePath == nil) {
            // We couldn't find the asset.  Send the appropriate error.
            [self cancelNSURLSessionTransferWithError:session errorMessage:[NSString stringWithFormat:@"Could not create target file"]];
            return;
        }
        ...
        if ([[GDFileManager defaultManager] createDirectoryAtPath:parentPath withIntermediateDirectories:YES attributes:nil error:&error] == NO) {
            if (error) {
                [self cancelNSURLSessionTransferWithError:session errorMessage:[NSString stringWithFormat:@"Could not create path to save downloaded file: %@", [error localizedDescription]]];
            } else {
                [self cancelNSURLSessionTransferWithError:session errorMessage:@"Could not create path to save downloaded file"];
            }
            return;
        }

        // create target file
        if ([[GDFileManager defaultManager] createFileAtPath:filePath contents:nil attributes:nil] == NO) {
           [self cancelNSURLSessionTransferWithError:session errorMessage:@"Could not create target file"];
            return;
        }

        // open target file for writing
        self.targetFileHandle = [GDFileHandle fileHandleForWritingAtPath:filePath];

        if (self.targetFileHandle == nil) {
            [self cancelNSURLSessionTransferWithError:session errorMessage:@"Could not open target file for writing"];
        }
        [GDCordovaLogger info:@"Streaming to file %@", filePath];

    } else if (self.responseCode == 304) {
        int error = 5;
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[command createFileTransferError:error AndSource:source AndTarget:target AndHttpStatus:self.responseCode AndBody:@""]];

        [self.command.commandDelegate sendPluginResult:result callbackId:callbackId];
    }
}

- (void)cancelNSURLSessionTransferWithError:(NSURLSession*)session errorMessage:(NSString*)errorMessage
{
    ...
    [self cancelSessionTask:_sessionTaskConnection];
    [self.command.commandDelegate sendPluginResult:result callbackId:callbackId];
}
