...
NSString* const kCDVAssetsLibraryPrefixBD = @"assets-library://";
NSString* const kCDVAssetsLibrarySchemeBD = @"assets-library";

@implementation CDVAssetLibraryFilesystemBD
@synthesize name=_name, urlTransformer;


/*
 The CDVAssetLibraryFilesystemBD works with resources which are identified
 by iOS as
   asset-library://<path>
...
- (NSURL *)assetLibraryURLForLocalURL:(CDVFilesystemURLBD *)url
{
    if ([url.url.scheme isEqualToString:kCDVFilesystemURLPrefixBD]) {
...
{
    NSString *path = nil;
    if ([[url.url scheme] isEqualToString:kCDVAssetsLibrarySchemeBD]) {
        path = [url.url path];
    } else {
       path = url.fullPath;
    }
...
- (void)getFileMetadataForURL:(CDVFilesystemURLBD *)localURL callback:(void (^)(CDVPluginResult *))callback
{
    ...
    ALAssetsLibraryAssetForURLResultBlock resultBlock = ^(ALAsset* asset) {
        if (asset) {
            ...
            [fileInfo setObject:[CDVAssetLibraryFilesystemBD getMimeTypeFromPath:filename] forKey:@"type"];
