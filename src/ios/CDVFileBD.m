...
NSString* const kCDVFilesystemURLPrefixBD = @"cdvfile";

@implementation CDVFilesystemURLBD
@synthesize url=_url;
...
{
    if ([[uri scheme] isEqualToString:kCDVFilesystemURLPrefixBD] && [[uri host] isEqualToString:@"localhost"]) {
    ...
    } else if ([[uri scheme] isEqualToString:kCDVAssetsLibrarySchemeBD]) {
...
- (NSString *)fullPathForLocalURI:(NSURL *)uri
{
    if ([[uri scheme] isEqualToString:kCDVFilesystemURLPrefixBD] && [[uri host] isEqualToString:@"localhost"]) {
        ...
        return [[uri absoluteString] substringFromIndex:[kCDVAssetsLibrarySchemeBD length]+2];
    }
    return nil;
}
...
{
    return [[CDVFilesystemURLBD alloc] initWithString:strURL];
}

+ (CDVFilesystemURLBD *)fileSystemURLWithURL:(NSURL *)URL
{
    return [[CDVFilesystemURLBD alloc] initWithURL:URL];
}
...
@implementation CDVFilesystemURLProtocolBD

+ (BOOL)canInitWithRequest:(NSURLRequest*)request
{
    NSURL* url = [request URL];
    return [[url scheme] isEqualToString:kCDVFilesystemURLPrefixBD];
}
...
- (void)registerFilesystem:(NSObject<CDVFileSystemBD> *)fs {
    __weak CDVFileBD* weakSelf = self;
...
- (NSObject<CDVFileSystemBD> *)fileSystemByName:(NSString *)fsName
{
    if (self.fileSystems != nil) {
        for (NSObject<CDVFileSystemBD> *fs in self.fileSystems) {
...
- (void)registerExtraFileSystems:(NSArray *)filesystems fromAvailableSet:(NSDictionary *)availableFileSystems
{
    ...
    for (NSString *fsName in filesystems) {
        if (![installedFilesystems containsObject:fsName]) {
            ...
            if (fsRoot) {
                [filePluginBD registerFilesystem:[[CDVLocalFilesystemBD alloc] initWithName:fsName root:fsRoot]];
...
- (void)pluginInitialize
{
    ...
    if ([[GDFileManager defaultManager] createDirectoryAtPath:self.appTempPath
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:&error]) {
        [self registerFilesystem:[[CDVLocalFilesystemBD alloc] initWithName:@"temporary" root:self.appTempPath]];
    } else {
        NSLog(@"Unable to create temporary directory: %@", error);
    }
    if ([location isEqualToString:@"library"]) {
        if ([[GDFileManager defaultManager] createDirectoryAtPath:self.appLibraryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error]) {
            [self registerFilesystem:[[CDVLocalFilesystemBD alloc] initWithName:@"persistent" root:self.appLibraryPath]];
        } else {
         ...
         *  versions.
         */
        [self registerFilesystem:[[CDVLocalFilesystemBD alloc] initWithName:@"persistent" root:self.rootDocsPath]];
    } else {
        NSAssert(false,
...
- (CDVFilesystemURLBD *)fileSystemURLforArg:(NSString *)urlArg
{
    CDVFilesystemURLBD* ret = nil;
    if ([urlArg hasPrefix:@"file://"]) {
        ...
        ret = [CDVFilesystemURLBD fileSystemURLWithString:urlArg];
    }
    return ret;
}
...
{
    CDVFilesystemURLBD *localURL = nil;
    ...
    for (id object in self.fileSystems) {
        if ([object respondsToSelector:@selector(URLforFilesystemPath:)]) {
            CDVFilesystemURLBD *url = [object URLforFilesystemPath:localPath];
            if (url){
...
{
    GDFileManager* fMgr = [[GDFileManager alloc] init];
...
- (void)requestFileSystem:(CDVInvokedUrlCommand*)command
{
    ...
    if (type >= self.fileSystems.count) {
        ...
        if (pNumAvail && ([pNumAvail unsignedLongLongValue] < size)) {
            ...
            NSObject<CDVFileSystemBD> *rootFs = [self.fileSystems objectAtIndex:type];
            if (rootFs == nil) {
                result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:NOT_FOUND_ERR];
                NSLog(@"No filesystem of type requested");
            } else {
...
- (void)requestAllFileSystems:(CDVInvokedUrlCommand*)command
{
    ...
    for (NSObject<CDVFileSystemBD>* root in fileSystems_) {
...
- (void)requestAllPaths:(CDVInvokedUrlCommand*)command
{
    ...
    [[GDFileManager defaultManager] createDirectoryAtPath:libPathSync withIntermediateDirectories:YES attributes:nil error:nil];
    [[GDFileManager defaultManager] createDirectoryAtPath:libPathNoSync withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    NSDictionary* ret = @{
        ...
        @"syncedDataDirectory": [[NSURL fileURLWithPath:libPathSync isDirectory:YES]absoluteString],
...
{
    NSObject<CDVFileSystemBD> *fs = [self fileSystemByName:fsName];
    return [fs makeEntryForPath:fullPath isDirectory:isDir];
}

- (NSDictionary *)makeEntryForLocalURL:(CDVFilesystemURLBD *)localURL
{
    NSObject<CDVFileSystemBD> *fs = [self filesystemForURL:localURL];
    return [fs makeEntryForLocalURL:localURL];
}

- (NSDictionary *)makeEntryForURL:(NSURL *)URL
{
    CDVFilesystemURLBD* fsURL = [self fileSystemURLforArg:[URL absoluteString]];
...
- (void)resolveLocalFileSystemURI:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFilesystemURLBD* inputURI = [self fileSystemURLforArg:localURIstr];
    ...
    if (inputURI == nil || inputURI.fileSystemName == nil) {
        ...
        NSObject<CDVFileSystemBD> *fs = [self filesystemForURL:inputURI];
        if (fs == nil) {
            result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:ENCODING_ERR];
        } else {
...
- (void)getFile:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFilesystemURLBD* baseURI = [self fileSystemURLforArg:baseURIstr];
...
- (void)getParent:(CDVInvokedUrlCommand*)command
{
    ...
    CDVFilesystemURLBD* localURI = [self fileSystemURLforArg:command.arguments[0]];

    NSObject<CDVFileSystemBD> *fs = [self filesystemForURL:localURI];
...
- (void)doCopyMove:(CDVInvokedUrlCommand*)command isCopy:(BOOL)bCopy
{
    ...
    CDVFilesystemURLBD* srcURL = [self fileSystemURLforArg:srcURLstr];
    CDVFilesystemURLBD* destURL = [self fileSystemURLforArg:destURLstr];

    NSObject<CDVFileSystemBD> *srcFs = [self filesystemForURL:srcURL];
    NSObject<CDVFileSystemBD> *destFs = [self filesystemForURL:destURL];
...
- (void)readAsText:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^ {
        [fs readFileAtURL:localURI start:start end:end callback:^(NSData* data, NSString* mimeType, CDVFileErrorBD errorCode) {
            CDVPluginResult* result = nil;
            if (data != nil) {
...
- (NSString *)filesystemPathForURL:(CDVFilesystemURLBD *)localURL
{
    for (NSObject<CDVFileSystemBD> *fs in self.fileSystems) {
...
- (void)testFileExists:(CDVInvokedUrlCommand*)command
{
    ...
    GDFileManager* fMgr = [GDFileManager defaultManager];
...
{
    return [CDVLocalFilesystemBD getMimeTypeFromPath:fullPath];
}

- (NSDictionary *)getDirectoryEntry:(NSString *)localPath isDirectory:(BOOL)bDirRequest
{
    CDVFilesystemURLBD *localURL = [self fileSystemURLforLocalPath:localPath];
...
{
    CDVFilesystemURLBD* localURL = [self fileSystemURLforArg:command.arguments[0]];
