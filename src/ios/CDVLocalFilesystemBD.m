...
- (NSDictionary *)makeEntryForLocalURL:(CDVFilesystemURLBD *)url {
    ...
    if (bExists && [path localizedCaseInsensitiveContainsString:@"www/storage/resources"] && !isDir)
    {
...
- (CDVFilesystemURLBD *)URLforFullPath:(NSString *)fullPath
{
    if (fullPath) {
        NSString* escapedPath = [fullPath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        if ([fullPath hasPrefix:@"/"]) {
            return [CDVFilesystemURLBD fileSystemURLWithString:[NSString stringWithFormat:@"%@://localhost/%@%@", kCDVFilesystemURLPrefixBD, self.name, escapedPath]];
        }
        return [CDVFilesystemURLBD fileSystemURLWithString:[NSString stringWithFormat:@"%@://localhost/%@/%@", kCDVFilesystemURLPrefixBD, self.name, escapedPath]];
    }
    return nil;
...
- (CDVPluginResult *)getFileForURL:(CDVFilesystemURLBD *)baseURI requestedPath:(NSString *)requestedPath options:(NSDictionary *)options
{
    ...
    if ([requestedPath rangeOfString:@":"].location != NSNotFound) {
        ...
        CDVFilesystemURLBD* requestedURL = [self URLforFullPath:combinedPath];
...
{
    CDVPluginResult* result = nil;
    CDVFilesystemURLBD *newURI = nil;
    if ([localURI.fullPath isEqualToString:@""]) {
        // return self
        newURI = localURI;
    } else {
        newURI = [CDVFilesystemURLBD fileSystemURLWithURL:[localURI.url URLByDeletingLastPathComponent]]; /* TODO: UGLY - FIX */
    }
    GDFileManager* fileMgr = [[GDFileManager alloc] init];
    BOOL bIsDir;
...
- (CDVPluginResult*)doRemove:(NSString*)fullPath
{
    ...
    @try {
        ...
        if (bSuccess) {
            ...
            CDVFileErrorBD errorCode = ABORT_ERR;
...
- (unsigned long long)truncateFile:(NSString*)filePath atPosition:(unsigned long long)pos
{
    ...
    GDFileHandle* file = [GDFileHandle fileHandleForWritingAtPath:filePath];
...
- (CDVPluginResult *)writeToFileAtURL:(CDVFilesystemURLBD *)localURL withData:(NSData*)encData append:(BOOL)shouldAppend
{
    ...
    CDVPluginResult* result = nil;
    CDVFileErrorBD errCode = INVALID_MODIFICATION_ERR;
    int bytesWritten = 0;
    ...
    if ([[GDFileManager defaultManager] fileExistsAtPath:filePath])
    {
        if (shouldAppend)
        {
            NSMutableData* currentContent = [[[GDFileManager defaultManager] contentsAtPath:filePath] mutableCopy];
            [currentContent appendData:encData];

            [[GDFileManager defaultManager] removeItemAtPath:filePath error:nil];

            [[GDFileManager defaultManager] createFileAtPath:filePath contents:currentContent attributes:[NSDictionary dictionary]];
        }
        else
        {
            [[GDFileManager defaultManager] removeItemAtPath:filePath error:nil];
            [[GDFileManager defaultManager] createFileAtPath:filePath contents:encData attributes:[NSDictionary dictionary]];
        }
    }
    else
    {
...
//    }
    return [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:(int)[encData length]];
}
...
- (void)copyFileToURL:(CDVFilesystemURLBD *)destURL withName:(NSString *)newName fromFileSystem:(NSObject<CDVFileSystemBD> *)srcFs atURL:(CDVFilesystemURLBD *)srcURL copy:(BOOL)bCopy callback:(void (^)(CDVPluginResult *))callback
{
    ...
    if (!bDestExists) {
    ...
    else if ([srcFs isKindOfClass:[CDVLocalFilesystemBD class]]) {
        ...
        [srcFs readFileAtURL:srcURL start:0 end:-1 callback:^(NSData* data, NSString* mimeType, CDVFileErrorBD errorCode) {
            CDVPluginResult* result = nil;
            if (data != nil) {
...
- (void)readFileAtURL:(CDVFilesystemURLBD *)localURL start:(NSInteger)start end:(NSInteger)end callback:(void (^)(NSData*, NSString* mimeType, CDVFileErrorBD))callback
{
    ...
    NSString* mimeType = [CDVLocalFilesystemBD getMimeTypeFromPath:path];
    if (mimeType == nil) {
        mimeType = @"*/*";
    }

    GDFileHandle* file = [GDFileHandle fileHandleForReadingAtPath:path];
    if (start > 0) {
    ...
    // old method
//    NSData *readData = [[GDFileManager defaultManager] contentsAtPath:path];
...
- (void)getFileMetadataForURL:(CDVFilesystemURLBD *)localURL callback:(void (^)(CDVPluginResult *))callback
{
    ...
    NSString* createErrorString = [NSString stringWithFormat:@"%@", error];
    if ([createErrorString containsString:@"No such file or directory"]) {
        fileAttrs = nil;
        NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : @"The file couldn’t be opened because there is no such file." };
        error = [NSError errorWithDomain:NSCocoaErrorDomain code:260 userInfo:userInfo];
    }
