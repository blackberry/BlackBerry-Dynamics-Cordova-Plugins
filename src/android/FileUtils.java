/*
       ...
       under the License.
 */
package com.blackberry.bbd.cordova.plugins.file;
 ...
 */
public class FileUtils extends GDBasePlugin {
    ...
    protected GDFileSystemDelegate gdFileSystem = new GDFileSystemDelegate();
    ...
    protected String[] getExtraFileSystemsPreference(Activity activity) {
        String fileSystemsStr = preferences.getString("androidextrafilesystems", "files,documents,cache,assets,root");
    ...
    protected void registerExtraFileSystems(String[] filesystems, HashMap<String, String> availableFileSystems) {
        ...
        for (String fsName : filesystems) {
            if (!installedFileSystems.contains(fsName)) {
                ...
                if (fsRoot != null) {
                    if (LocalFilesystem.prepareDirectory(fsRoot)) {
                        registerFilesystem(new LocalFilesystem(fsName, webView.getContext(), webView.getResourceApi(), fsRoot));
    ...
    protected HashMap<String, String> getAvailableFileSystems(Activity activity) {
        ...
        availableFileSystems.put("documents", new java.io.File(context.getFilesDir(), "Documents").getAbsolutePath());
        availableFileSystems.put("cache", context.getCacheDir().getAbsolutePath());
        availableFileSystems.put("root", "/");

        // we don't support external storage
    ...
    private static String toDirUrl(java.io.File f) {
   ...
   /**
     ...
     * @throws JSONException
     */
    public JSONObject getEntryForFile(com.good.gd.file.File file) throws JSONException {
        JSONObject entry;
     ...
     */
    @Deprecated
    public static JSONObject getEntry(com.good.gd.file.File file) throws JSONException {
    ...
    /*
     * get file system metadata by nativeUri
     */

    public JSONObject getFilesystemMetadata(Uri nativeUri) throws JSONException {

        final String nativeUriStr = nativeUri.toString();

        // Try all installed filesystems. Return the matching File System MetaData
        int type = 0;
        for (Filesystem fs: filesystems) {
            final boolean local = (fs instanceof LocalFilesystem);
            final Uri rootUri = fs.getRootUri();
            final String rootUriStr = rootUri.toString();
            final String name = fs.name;
            if (nativeUriStr.startsWith(rootUriStr)) {
                JSONObject fsMetaData = new JSONObject();
                fsMetaData.put("name", name);
                fsMetaData.put("type", type);
                fsMetaData.put("local", local);
                fsMetaData.put("rootUri", rootUriStr);
                fsMetaData.put("localPath", nativeUriStr.substring(rootUriStr.length()));
                return fsMetaData;
            }
            type++;
        }
