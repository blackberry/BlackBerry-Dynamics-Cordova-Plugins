...
public abstract class Filesystem {
    ...
    public InputStream getInputStreamForURL(LocalFilesystemURL inputURL) throws IOException {
    ...
    public void readFileAtURL(LocalFilesystemURL inputURL, long start, long end,
        ...
        InputStream inputStream = getInputStreamForURL(inputURL);
        if (end < 0 || end > inputStream.available()) {
        ...
        try {
            ...
            if (numBytesToRead < inputStream.available()) {
                inputStream = new LimitedInputStream(inputStream, numBytesToRead);
            }
            readFileCallback.handleData(inputStream, resourceApi.getMimeType(toNativeUri(inputURL)));
