...
public class LocalFilesystem extends Filesystem {
    ...
    public LocalFilesystem(String name, Context context, CordovaResourceApi resourceApi, String fsRoot) {
        super(Uri.fromFile(new File(fsRoot)).buildUpon().appendEncodedPath("").build(), name, resourceApi);
	...
	public JSONObject getFileForLocalURL(LocalFilesystemURL inputURL,
        ...
        if (create && exclusive && fp.exists()) {
            throw new FileExistsException("create/exclusive fails");
        } else if (!fp.exists() && create) {
            if (directory) {
                fp.mkdir();
            } else {
                fp.createNewFile();
    ...
    public long getFreeSpaceInBytes() {
        File file = new File(rootUri.getPath());
	...
	protected boolean removeDirRecursively(File directory) throws FileExistsException {
        if (directory.isDirectory()) {
            for (java.io.File file : directory.listFiles()) {
                removeDirRecursively((com.good.gd.file.File)file);
            }
        }
    ...
    public LocalFilesystemURL[] listChildren(LocalFilesystemURL inputURL) throws FileNotFoundException {
        ...
        java.io.File[] files = fp.listFiles();
        if (files == null) {
    ...
    private void copyFile(Filesystem srcFs, LocalFilesystemURL srcURL, File destFile, boolean move) throws IOException, InvalidModificationException, NoModificationAllowedException {
        ...
        InputStream inputStream = srcFs.getInputStreamForURL(srcURL);
        copyResource(inputStream, new FileOutputStream(destFile));
    ...
    private boolean isPublicDirectory(String absolutePath) {
        // all writable storage should be encrypted in this plugin, so public directory is not supported
        return false;
    }
    ...
    @Override
    public java.io.InputStream getInputStreamForURL(LocalFilesystemURL inputURL) throws IOException {
        return new FileInputStream(toNativeUri(inputURL).getPath());
    }
    ...
    private static void copyResource(InputStream inputStream, FileOutputStream outputStream) throws IOException {
        try {
